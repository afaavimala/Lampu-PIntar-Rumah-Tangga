...
if ! load_root_env "env-local"; then
  echo "[env-local] Missing root .env. Create it first: cp .env.example .env"
  exit 1
fi
...
apply_env_mappings "env-local" "$BACKEND_ENV_FILE" \
  ...
  "AUTH_LOGIN_BACKOFF_BASE_SEC:BACKEND_AUTH_LOGIN_BACKOFF_BASE_SEC" \
  "AUTH_LOGIN_BACKOFF_FACTOR:BACKEND_AUTH_LOGIN_BACKOFF_FACTOR" \
  "AUTH_LOGIN_BACKOFF_MAX_SEC:BACKEND_AUTH_LOGIN_BACKOFF_MAX_SEC" \
  ...
  "SEED_SAMPLE_DEVICE_ENABLED:BACKEND_SEED_SAMPLE_DEVICE_ENABLED" \
